...
load("Final-Figs/dat_multi_12_28.Rdata")
head(dat.multi)
...
dat.multi <- subset(dat.multi, day<=500)
unique(dat.multi$test_rotation) #this is correct


unique(dat.multi$test_rotation)
dat.plot = subset(dat.multi, test_rotation=="all-no-test" | test_rotation=="two-week-all" | test_rotation=="semi-weekly-high-three-week-low" | test_rotation=="semi-weekly-all" | test_rotation=="weekly-all")

dat.plot$test_rotation = factor(dat.plot$test_rotation, levels = c("semi-weekly-all", "weekly-all", "semi-weekly-high-three-week-low",  "two-week-all", "all-no-test"))
colz = c("semi-weekly-all" = "indianred1", "semi-weekly-high-three-week-low" = "cyan2", "two-week-all" = "springgreen3", "weekly-all" = "purple",  "all-no-test" = "gray50")
...
p5b <- ggplot(data=dat.plot) + geom_line(aes(x=day, y=mean_cumulative, color=test_rotation)) + scale_color_manual(values=colz) + 
  ...
  theme(panel.grid  = element_blank(), axis.title = element_text(size=14), axis.text = element_text(size=12),
        legend.title = element_blank(), legend.position = c(.7,.23), strip.background = element_blank(),
...
dat.sum$test_rotation<- factor(dat.sum$test_rotation, levels=c("semi-weekly-all", "weekly-all", "semi-weekly-high-three-week-low", "semi-weekly-high-two-week-low", "two-week-all"))

dat.sum$label <- as.character(dat.sum$test_rotation)
dat.sum$label[dat.sum$label=="semi-weekly-high-three-week-low"] <- "semi-weekly-high\nthree-week-low"
dat.sum$label <- factor(dat.sum$label, levels = c("semi-weekly-all", "weekly-all", "semi-weekly-high\nthree-week-low", "semi-weekly-high-two-week-low", "two-week-all") )
...
load("Final-Figs/R0_multi_group_12_28.Rdata")
head(dat.multi.R0)

dat.multi.R0 <- arrange(dat.multi.R0, desc(mean))


dat.multi.R0.plot = subset(dat.multi.R0, test_rotation=="semi-weekly-all" | test_rotation== "weekly-all" | test_rotation== "semi-weekly-high-three-week-low" | test_rotation== "two-week-all" )

dat.multi.R0.plot$test_rotation <- factor(dat.multi.R0.plot$test_rotation, levels=c("semi-weekly-all",  "weekly-all", "semi-weekly-high-three-week-low",  "two-week-all"))
