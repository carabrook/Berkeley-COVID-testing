...
#high vax high R0
make_FigS8 <- function(filename){
load("Final-Figs/dat.test.all.10.26.high.vax.high.R0.Rdata")
head(dat.test.all)
...
dat.none=subset(dat.test.all, test_rotation=="none")
dat.test.all =subset(dat.test.all, test_rotation!="none")
...
dat.none = subset(dat.none, intervention_class=="none" & day<=50)
...
dat.sum$cases_saved_lci[dat.sum$cases_saved_lci<0]<- 0
...
p4b  <- ggplot(dat.sum.plot) + xlab("") + ylab("cases saved") + scale_fill_manual(values = newcolz) +  scale_color_manual(values = newcolz) +
  ...
  geom_errorbar(aes(x=test_rotation, ymin=cases_saved_lci, ymax=cases_saved_uci,color = intervention_class ), width =.3, size=.5, 
                position=position_dodge(width = .9), show.legend = F) + coord_cartesian(ylim=c(0,200)) +
...
load("Final-Figs/R0_test_all_group_10_26_high_vax_high_R0.Rdata")
...
dat.test.R0$mean[dat.test.R0$mean<0]<- 0
dat.test.R0$lci[dat.test.R0$lci<0]<- 0
dat.test.R0$uci[dat.test.R0$uci<0]<- 0

#split 
R0.split <- dlply(dat.test.R0, .(intervention_class, test_rotation, n_test_days))
#and try the diff again

get.diff <- function(df){
  diff.mean <- df$mean[df$class=="UCB_potential_cases"] - df$mean[df$class=="UCB_actual_cases"]
  diff.lci <- df$lci[df$class=="UCB_potential_cases"] - df$lci[df$class=="UCB_actual_cases"]
  diff.uci <- df$uci[df$class=="UCB_potential_cases"] - df$uci[df$class=="UCB_actual_cases"]
  
  df$mean[df$class=="diff_post_intervention"] <- diff.mean
  df$lci[df$class=="diff_post_intervention"] <- diff.lci
  df$uci[df$class=="diff_post_intervention"] <- diff.uci
  
  return(df)
  
}
R0.split <- lapply(R0.split, get.diff)

dat.test.R0 <- data.table::rbindlist(R0.split)

dat.test.R0.plot = subset(dat.test.R0, n_test_days==2 & class=="diff_post_intervention")
...
p.4a <- ggplot(data=dat.test.R0.plot) + geom_point(aes(x=test_rotation, y=mean, color=intervention_class), shape = 16, size=5,  position=position_dodge(width=.5)) + 
  ylab("reduction in Reff from intervention") + scale_color_manual(values = newcolz2) + #coord_cartesian(ylim=c(0,4)) +
  ...
  theme_bw() + theme(panel.grid = element_blank(), axis.text= element_text(size = 12), axis.title = element_text(size=14), strip.text = element_text(size=14),
                     ...
                     legend.title = element_blank(), legend.position = c(.53,.62), legend.text = element_text(size=8)) 
#
...
make_FigS8(filename="Final-Figs/FigS8.png")
