...
make_FigS7 <- function(filename){
load("Final-Figs/dat.test.all.10.26.low.vax.high.R0.Rdata")
head(dat.test.all)
...
#dat.test.all$test_rotation[dat.test.all$test_rotation=="none"] <- "no testing"

dat.test.all =subset(dat.test.all, test_rotation!="none")
dat.test.all$test_rotation = factor(dat.test.all$test_rotation, levels = c("semi-weekly testing", "weekly testing", "every two week testing"))#, "no-testing"))
...
#and no intervention by compariosn
psupp <- ggplot(data=dat.none) + scale_color_manual(values = colz) + #scale_fill_manual(values = colz) +
  geom_ribbon(aes(x=day,ymin=lci_exposures, ymax=uci_exposures),  alpha=.3) + #ylim(0,350) +
  geom_line(data=leg.dat, aes(x=tmp1, y=tmp2, color=type)) + 
  #geom_ribbon(data=leg.dat, aes(x=tmp1, ymin=tmp2, ymax=tmp2, fill=type)) +
  #geom_ribbon(aes(x=day,ymin=lci_isolations, ymax=uci_isolations), fill="red", alpha=.3) +
...
p4b  <- ggplot(dat.sum.plot) + xlab("") + ylab("cases saved") + scale_fill_manual(values = newcolz) +  scale_color_manual(values = newcolz) +
  ...
  geom_errorbar(aes(x=test_rotation, ymin=cases_saved_lci, ymax=cases_saved_uci,color = intervention_class ), width =.3, size=.5, 
                position=position_dodge(width = .9), show.legend = F) + coord_cartesian(ylim=c(0,5000)) +
...
load("Final-Figs/R0_test_all_group_10_26_low_vax_high_R0.Rdata")
...
make_FigS7(filename="Final-Figs/FigS7.png")
