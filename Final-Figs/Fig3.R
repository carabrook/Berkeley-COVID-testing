...
dat.sum.testing$test_rotation[dat.sum.testing$test_rotation=="biweekly"] <- "semi-weekly-testing"
...
dat.sum.testing$test_rotation <- factor(dat.sum.testing$test_rotation, levels = c("semi-weekly-testing", "weekly-testing", "every-two-weeks-testing"))

#plus cases saved:
alphaz <- c("1e+01" = 1, "1e+03" = .7, "1e+05"=.5)
...
p1a2 <- ggplot(dat.sum.testing)  + ylab("cases saved") +  facet_grid(~test_rotation)  + scale_alpha_manual(values=alphaz) + #ggtitle("testing interventions")
  scale_fill_manual(values=fillz) + guides(fill="none") +  geom_bar(aes(x=TAT, y=cases_saved, alpha=LOD, fill = TAT), stat = "identity", position=position_dodge(),  color="black", show.legend = F) + coord_cartesian(ylim=c(-1000,18000)) +
...
p1c2 <- ggplot(dat.sum.group) + xlab("group size limit (# persons)") + ylab("cases saved") +  facet_grid(~intervention_class) + scale_fill_manual(values = fillz2) + scale_color_manual(values = fillz2) +
  geom_bar(aes(x=distance_limit, y=cases_saved, fill=distance_limit), stat = "identity", position=position_dodge(),  color="black", show.legend = F) + coord_cartesian(ylim=c(-1000,18000)) +
...
dat.test.R0.testing$test_rotation[dat.test.R0.testing$test_rotation=="biweekly"] <- "semi-weekly-testing"
...
dat.test.R0.testing$test_rotation <- factor(dat.test.R0.testing$test_rotation, levels = c("semi-weekly-testing", "weekly-testing", "every-two-weeks-testing"))
...
make.SuppFile2 <- function(filename){
...
make.SuppFile2(filename = "Final-Figs/Supplementary-File-2.csv")
