...
#and get cases saved
dat.sum$cases_saved = dat.sum$total_cases[dat.sum$intervention_class=="none" & dat.sum$prop_asym==.49] - dat.sum$total_cases 
dat.sum$cases_saved_lci = dat.sum$lci_cases[dat.sum$intervention_class=="none" & dat.sum$prop_asym==.49] - dat.sum$lci_cases 
dat.sum$cases_saved_uci = dat.sum$uci_cases[dat.sum$intervention_class=="none" & dat.sum$prop_asym==.49] - dat.sum$uci_cases 
...
dat.sum$label = NA
dat.sum$label[dat.sum$prop_asym==.3] <- "30% asymptomatic"
dat.sum$label[dat.sum$prop_asym==.49] <- "50% asymptomatic"
...
dat.test.R0$label[dat.test.R0$prop_asym==.3] <- "30% asymptomatic"
dat.test.R0$label[dat.test.R0$prop_asym==.49] <- "50% asymptomatic"
...
p.A <- ggplot(data=dat.test.R0.symptom) + geom_point(aes(x=iso_lag, y=mean, color=iso_lag),  shape = 15, size=5, show.legend = F) +
  scale_color_manual(values=colorz) + scale_alpha_manual(values=alphaz) + facet_grid(label~intervention_class) + guides(color="none", shape="none") +
