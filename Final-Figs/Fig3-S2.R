...
make_Fig3_S2 <- function(filename){
  
load("Final-Figs/dat.trace.12.28.Rdata")

head(dat.trace)
dat.trace$test_rotation <- sapply(strsplit(dat.trace$test_rotation, "-"), '[', 1)
dat.trace$test_rotation[dat.trace$test_rotation=="biweekly"] <- "semi-weekly-testing"
...
dat.trace$test_rotation <- factor(dat.trace$test_rotation, levels = c("semi-weekly-testing", "weekly-testing", "every-two-weeks-testing"))
...
dat.sum.test$test_rotation = factor(dat.sum.test$test_rotation, levels = c("semi-weekly-testing", "weekly-testing", "every-two-weeks-testing"))
...
p1b <- ggplot(dat.sum.test)  + ylab("cases saved")   + facet_grid(label~test_rotation) +  scale_fill_manual(values=fillz) + scale_color_manual(values=fillz) + guides(fill="none") +
  geom_bar(aes(x=TAT, y=cases_saved, fill = TAT), stat = "identity", position=position_dodge(),  color="black", show.legend = F) + coord_cartesian(ylim=c(-1000,18000)) +
...
load("Final-Figs/R0.trace.12.28.Rdata")

dat.trace.R0$test_rotation <- sapply(strsplit(dat.trace.R0$test_rotation, "-"), '[', 1)
dat.trace.R0$test_rotation[dat.trace.R0$test_rotation=="biweekly"] <- "semi-weekly-testing"
...
dat.trace.R0$test_rotation <- factor(dat.trace.R0$test_rotation, levels = c("semi-weekly-testing", "weekly-testing", "every-two-weeks-testing"))
...
p.2b <- ggplot(data=dat.symp) + geom_point(aes(x=iso_lag, y=mean, color=iso_lag), shape = 16, size=5,show.legend = F,  position=position_dodge(width=.5)) + 
  scale_color_manual(values=fillz) + 
  ylab("reduction in Reff from intervention") + xlab("lag to isolation (days)") + facet_grid(label~intervention_class) +
...
make_Fig3_S2("Final-Figs/Fig3-S2.png")


#and the second data set
make.SuppFile3 <- function(filename){
  ...
  dat.sum$LOD = factor(dat.sum$LOD, levels = c("1e+01", "1e+03", "1e+05"))
  ...
  dat.trace.R0$LOD = factor(dat.trace.R0$LOD, levels = c("1e+01", "1e+03", "1e+05"))
  ...
  dat.sum$test_rotation[dat.sum$test_rotation=="biweekly"] <- "semi-weekly-testing"
  ...
  dat.sum$test_rotation <- factor(dat.sum$test_rotation, levels = c("semi-weekly-testing", "weekly-testing", "every-two-weeks-testing", "none"))
...
make.SuppFile3(filename = "Final-Figs/Supplementary-File-3.csv")
