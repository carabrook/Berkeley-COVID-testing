...
#and alt
label.dat <- data.frame(rbind(rbind(c("group size limits", 27,.4))))
...
p2B2 <- ggplot(data=group.dat) + scale_color_manual(values=colorz) + scale_fill_manual(values=colorz) +
  ...
  geom_line(aes(x=day, y= mean_exposures), color="black", show.legend = F) + theme_bw() + coord_cartesian(ylim=c(0,700)) +
