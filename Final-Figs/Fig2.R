...
make_Figure_2 <- function(filename){
...
label.dat <- data.frame(rbind(c("superspreader threshold", 10,.6),c("group limits", 10,.4)))
...
#and alt
label.dat$label[2] <- "group size limits"
label.dat$x[2] <- 27
label.dat$x[1] <- 13
...
p2B2 <- ggplot(data=group.dat) + scale_color_manual(values=colorz) + scale_fill_manual(values=colorz) +
  ...
  geom_line(aes(x=day, y= mean_exposures), color="black", show.legend = F) + theme_bw() + coord_cartesian(ylim=c(0,300)) +
...
p2D2 <- ggplot(data=group.dat) + geom_line(aes(x=day, y=mean_cumulative), color="black", show.legend = F) + scale_color_manual(values=colorz) + scale_fill_manual(values=colorz) +
  geom_ribbon(aes(x=day, ymin=lci_cumulative, ymax=uci_cumulative, fill=distance_limit), alpha=.3, show.legend = F) + coord_cartesian(ylim=c(0,10000)) +
