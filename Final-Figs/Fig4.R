...
dat.test.all$test_rotation[dat.test.all$test_rotation=="biweekly"] <- "biweekly testing"
...
dat.test.all$test_rotation = factor(dat.test.all$test_rotation, levels = c("biweekly testing", "weekly testing", "every two week testing"))
unique(dat.test.all$test_rotation) 
unique(dat.test.all$n_test_days)

dat.test.all$intervention_class <- NA
dat.test.all$intervention_class[dat.test.all$iso_lag==Inf  & dat.test.all$trace_lag==Inf & dat.test.all$distance_limit==Inf] <- "testing only"
dat.test.all$intervention_class[dat.test.all$iso_lag==Inf  & dat.test.all$trace_lag==1 & dat.test.all$distance_limit==Inf] <- "test + trace"
dat.test.all$intervention_class[dat.test.all$iso_lag==1  & dat.test.all$trace_lag==1 & dat.test.all$distance_limit==Inf] <- "test + trace + symptom-iso"
dat.test.all$intervention_class[dat.test.all$iso_lag==1  & dat.test.all$trace_lag==1 & dat.test.all$distance_limit==12] <- "test + trace +\nsymptom-iso + group limit"

dat.test.all$intervention_class = factor(dat.test.all$intervention_class,levels = c("test + trace +\nsymptom-iso + group limit","test + trace + symptom-iso", "test + trace", "testing only"))
...
p4b  <- ggplot(dat.sum.plot) + xlab("") + ylab("cases saved") + scale_fill_manual(values = newcolz) +  scale_color_manual(values = newcolz) +
  ...
  geom_errorbar(aes(x=test_rotation, ymin=cases_saved_lci, ymax=cases_saved_uci,color = intervention_class ), width =.3, size=.5, 
                position=position_dodge(width = .9), show.legend = F) + coord_cartesian(ylim=c(0,12000)) +
...
make.Table.S3 <- function(filename){
  ...
  dat.test.all$intervention_class[dat.test.all$iso_lag==Inf  & dat.test.all$trace_lag==1 & dat.test.all$distance_limit==Inf] <- "test + trace"
  dat.test.all$intervention_class[dat.test.all$iso_lag==1  & dat.test.all$trace_lag==1 & dat.test.all$distance_limit==Inf] <- "test + trace + symptom-iso"
  dat.test.all$intervention_class[dat.test.all$iso_lag==1  & dat.test.all$trace_lag==1 & dat.test.all$distance_limit==12] <- "test + trace + symptom-iso + group limit"
  
  dat.test.all$intervention_class = factor(dat.test.all$intervention_class,levels = c("test + trace + symptom-iso + group limit","test + trace + symptom-iso", "test + trace", "testing only"))
  ...
  dat.test.R0$test_rotation[dat.test.R0$test_rotation=="biweekly"] <- "biweekly testing"
  ...
  dat.test.R0$test_rotation = factor(dat.test.R0$test_rotation, levels = c("biweekly testing", "weekly testing", "every two week testing"))
  ...
  dat.sum$test_rotation[dat.sum$test_rotation=="biweekly testing"] <- "biweekly-testing"
  ...
  dat.sum$test_rotation = factor(dat.sum$test_rotation, levels=c( "biweekly-testing", "weekly-testing", "every-two-week-testing"))
