...
setwd("/Users/caraebrook/Documents/R/R_repositories/Berkeley-Reopening/Dec-2020/all-runs/Fig4-Layered-Interventions/out/")
...
dat8$iso_lag = Inf
dat8$trace_lag = Inf
load("weekly-7-test-days-trace-1-symp-group.Rdata")
dat9 <- out[[1]]
dat9$iso_lag = 1
dat9$trace_lag = 1
load("weekly-7-test-days-trace-1-symp.Rdata")
dat10 <- out[[1]]
dat10$iso_lag = 1
dat10$trace_lag = 1
load("weekly-7-test-days-trace-1.Rdata")
dat11 <- out[[1]]
dat11$iso_lag = Inf
dat11$trace_lag = 1
...
unique(dat$n_test_days)

dat$test_rotation[dat$test_rotation=="biweekly-2-test-days"] <- "semi-weekly"
...
dat$test_rotation[dat$test_rotation=="biweekly-5-test-days"] <- "semi-weekly"
...
dat$test_rotation[dat$test_rotation=="biweekly-7-test-days"] <- "semi-weekly"
...
dat$test_rotation = factor(dat$test_rotation, levels = c("semi-weekly", "weekly", "two-week"))
...
#add in no intervention comparison
...
dat.test.all <- rbind(dat.test.all, dat1)

dat.test.all$intervention_class = factor(dat.test.all$intervention_class,levels = c("test + trace +\nsymptom-iso + group limit","test + trace + symptom-iso", "test + trace", "testing only", "none"))

#and save and plot.
save(dat.test.all, file = "dat.test.all.12.28.Rdata")
...
dat.test.R0 <- rbind(dat.test.R0, dat1)
...
dat.test.R0$test_rotation <- factor(dat.test.R0$test_rotation, levels = c("semi-weekly", "weekly", "two-week", "none"))

unique(dat.test.R0$test_rotation)


#and save and plot.
save(dat.test.R0, file = "R0_test_all_group_12_28.Rdata")
