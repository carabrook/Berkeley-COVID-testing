...
#setwd("/Users/caraebrook/Documents/R/R_repositories/Berkeley-Reopening/test-sensitivity/multi-group-only-test/")
...
assign.ID = function(sus.dat.sub, dat.new.sub){
  ...
  if(nrow(dat.new.sub)<=length(sus.dat.sub$employ_ids)){
    ...
    new.count = length(sus.dat.sub$employ_ids) #all possible susceptibles
    new.missed = nrow(dat.new.sub) - new.count #required infections - all possible susceptibles. if there are more infections than suscept. this will be positive
...
multi.par$par1[multi.par$parameter=="test-rotation" & multi.par$FAST.enrolled=="yes"] <- "two-week"
...
multi.par$par1[multi.par$parameter=="test-freq"& multi.par$FAST.enrolled=="yes"] <- 14
...
out = replicate.epidemic(n.reps = 100,
                         ...
                         LOD=(10^1),
                         titer.dat = titer.dat,
                         test_rotation_name = "two-week-all")

save(out, file = "two-week-all.Rdata")
