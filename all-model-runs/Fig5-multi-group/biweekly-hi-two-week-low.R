...
#setwd("/Users/caraebrook/Documents/R/R_repositories/Berkeley-Reopening/test-sensitivity/multi-group/")
...
assign.infections <- function(pop.mat, gen_list, timestep, input.par){
  ...
  if(sum(new.mat$actual_cases_caused)>0){
...
get.actual.cases = function(pop.dat, dat.gen, timestep, weekend.amp){
  ...
  #then, if this is a weekend, you do go on to engender this many transmissions
  if(timestep ==1 | timestep ==2 | (timestep%%7==1)| (timestep%%7==2)){
    return(n.out) 
  }else{ #otherwise, you lose a few, stochastically
    n.new <- floor(n.out/add.noise(weekend.amp))
    return(n.new)
  }
}
...
get.gen = function(dat, genTime){
  ...
  names(dat.out) = c("employ_ids", "generation_time")
  return(dat.out)
}
...
get.gen.super = function(dat, genTime, thresh, dist.limit, perc.inf.super){
  ...
  if(dat$potential_cases_caused>0){
    ...
    #we assume that HALF the people get infected at a given superspreading event.
    #so delete all events with cases caused that exceed 2x this limit
    ...
    if(dist.limit.obs==TRUE & n.case.super>(perc.inf.super*dist.limit) & n.case.disp>0){ 
      ...
      rep.name = rep(dat$employ_ids,n.case.disp)
      rep.gen = sort(genTime(n.case.disp))
      
      #and, based on gentime, determine individually whether this event happens or not
      ...
      #there is an intervention but there are no superspread cases or
      #the superspreading event is below the intervention threshold so it still occurs
      #rep name stays the same
      rep.gen1 = genTime(n.case.disp)
      rep.gen2 = rep(genTime(1), n.case.super)#here is where you would add in multiple superspreading events if you wanted
      rep.gen = sort(c(rep.gen1, rep.gen2))  #the rest get 
    }
...
initiate.pop <- function(start.ID.employ, pop.UCB, n.init.exposed, pop.ID, input.par,  R0fn, genTime, superspread, titer.dat, LOD){
  ...
  if ((test_rotation=="biweekly" & n.test.day.per.wk==2) | (test_rotation=="weekly" & n.test.day.per.wk==2)){
    pop.mat$time_of_next_test = rep(c(3,7), length=pop.UCB) #Monday/Friday, then add 3 days for all until next test
    
  }else if((test_rotation=="biweekly" & n.test.day.per.wk==5) | (test_rotation=="weekly" & n.test.day.per.wk==5)){
    
    pop.mat$time_of_next_test = rep(c(3,4,5,6,7), length=pop.UCB)#Monday-Friday, then add 4 days for all until next test
    
  }else if((test_rotation=="biweekly" & n.test.day.per.wk==7) | (test_rotation=="weekly" & n.test.day.per.wk==7)){
    pop.mat$time_of_next_test = rep(seq(1,7,1), length=pop.UCB)
    
  }else if(test_rotation=="two-week" & n.test.day.per.wk==7){
    
    pop.mat$time_of_next_test = rep(c(seq(1,14,1)), length=pop.UCB)
    
  }else if(test_rotation=="two-week" & n.test.day.per.wk==5){
    
    pop.mat$time_of_next_test = rep(c(seq(3,7,1), seq(10,14)), length=pop.UCB)
    
  }else if(test_rotation=="two-week" & n.test.day.per.wk==2){
    
    pop.mat$time_of_next_test = rep(c(3,7, 10,14), length=pop.UCB)
    
  }else if(test_rotation=="three-week" & n.test.day.per.wk==2){
    pop.mat$time_of_next_test = rep(c(3,7, 10,14,17,21), length=pop.UCB)
  }
  ...
  pop.mat$reason_isolated[pop.mat$state==1 & pop.mat$time_isolation> pop.mat$time_of_testing_iso & pop.mat$testing==TRUE & pop.mat$time_of_next_test>=pop.mat$time_test_sensitive_start & pop.mat$time_of_next_test<pop.mat$time_test_sensitive_end & complete.cases(pop.mat)] <- "testing_iso"
...
epidemic.step = function(pop.mat, timestep,  length_timestep, prob.out,  gen_list, input.par){
  ...
  #now, compute actual next test day - add different frequencies depending on the type
  ...
  #first, assume that isolation time is symptomatic
  #print("1")
  pop.mat$reason_isolated[pop.mat$state==3  & pop.mat$exposure_time<=timestep  ] <- "symptom_iso"
  # print("2")
  pop.mat$time_isolation[pop.mat$state==3  & pop.mat$exposure_time<=timestep ] <- pop.mat$time_of_symptom_iso[pop.mat$state==3  & pop.mat$exposure_time<=timestep ]
  pop.mat$time_isolation = as.numeric(pop.mat$time_isolation)
  
  
  #now, if tracing comes first, we replace it
  #print("3")
  pop.mat$reason_isolated[pop.mat$state==3  & pop.mat$exposure_time<=timestep & pop.mat$time_of_tracing_iso<pop.mat$time_isolation & pop.mat$traced==TRUE ] <- "tracing_iso"
  # print("4")
  pop.mat$time_isolation[pop.mat$state==3  & pop.mat$exposure_time<=timestep & pop.mat$time_of_tracing_iso<pop.mat$time_isolation & pop.mat$traced==TRUE ] <- pop.mat$time_of_tracing_iso[pop.mat$state==3  & pop.mat$exposure_time<=timestep & pop.mat$time_of_tracing_iso<pop.mat$time_isolation & pop.mat$traced==TRUE ]
  
  
  #finally, if testing comes first, we replace it
  #print("5")
  pop.mat$reason_isolated[pop.mat$state==3  & pop.mat$exposure_time<=timestep & pop.mat$time_of_testing_iso<pop.mat$time_isolation & pop.mat$testing==TRUE & pop.mat$time_of_next_test>=pop.mat$time_test_sensitive_start & pop.mat$time_of_next_test<pop.mat$time_test_sensitive_end] <- "testing_iso"
  #print("6")
...
simulate.epidemic <- function(input.pop, n.init.exposed.vector, employ.id.vector, times, virus.par, input.par, burnin, serial_mean, serial_sd, test.freq, length_timestep, bay.area.prev, bay.area.R, superspread, titer.dat, LOD, test_rotation_name){
  ...
  out.list = mapply(FUN=initiate.pop, start.ID.employ = as.list(employ.id.vector), pop.UCB=as.list(input.pop), n.init.exposed= as.list(n.init.exposed.vector),  pop.ID = as.list(pop.num), titer.dat = titer.dat.split,
                    MoreArgs= list(input.par=input.par,  R0fn=R0fn,  genTime=genTime, superspread=superspread, LOD=LOD))
...
replicate.epidemic = function(n.reps, input.pop, n.init.exposed.vector, employ.id.vector, times, virus.par, input.par, burnin, serial_mean, serial_sd, test.freq, length_timestep,
  ...
  mean.dat$LOD <- LOD
  #mean.dat$TAT <- min(input.par$par1[input.par$parameter=="TAT-lag"])
  ...
  mean.long$LOD <- LOD
  #mean.long$TAT <- min(input.par$par1[input.par$parameter=="TAT-lag"])
  ...
  mean.R.mat$LOD <- LOD
  #mean.R.mat$TAT <- min(input.par$par1[input.par$parameter=="TAT-lag"])
...
multi.par$par1[multi.par$parameter=="iso-lag"] <- Inf
multi.par$par2[multi.par$parameter=="iso-lag"] <- 0
multi.par$par1[multi.par$parameter=="trace-lag"] <- Inf
multi.par$par2[multi.par$parameter=="trace-lag"] <- Inf
multi.par$par1[multi.par$parameter=="prop.trace"] <- 0
multi.par$par1[multi.par$parameter=="group-size-limit"] <- Inf
multi.par$par1[multi.par$parameter=="percent-obs-dist-lim"] <- 0

multi.par$par1[multi.par$parameter=="TAT-lag" & multi.par$FAST.enrolled=="yes"] <- 2
multi.par$par2[multi.par$parameter=="TAT-lag" & multi.par$FAST.enrolled=="yes"] <- .5
multi.par$par1[multi.par$parameter=="TAT-lag" & multi.par$FAST.enrolled=="no"] <- Inf
multi.par$par2[multi.par$parameter=="TAT-lag" & multi.par$FAST.enrolled=="no"] <- Inf
multi.par$par1[multi.par$parameter=="test-rotation" & multi.par$FAST.enrolled=="no"] <- "none"
multi.par$par1[multi.par$parameter=="n-test-days-per-week" & multi.par$FAST.enrolled=="no"] <- NA
multi.par$par1[multi.par$parameter=="test-rotation" & multi.par$FAST.enrolled=="yes" & multi.par$risk.group=="high"] <- "biweekly"
multi.par$par1[multi.par$parameter=="test-rotation" & multi.par$FAST.enrolled=="yes" & multi.par$risk.group=="low"] <- "two-week"
multi.par$par1[multi.par$parameter=="n-test-days-per-week" & multi.par$FAST.enrolled=="yes"] <- 2
multi.par$par1[multi.par$parameter=="test-on" & multi.par$FAST.enrolled=="yes"] <- TRUE
multi.par$par1[multi.par$parameter=="test-on" & multi.par$FAST.enrolled=="no"] <- FALSE
multi.par$par1[multi.par$parameter=="test-freq"& multi.par$FAST.enrolled=="yes" & multi.par$risk.group=="high"] <- 3
multi.par$par1[multi.par$parameter=="test-freq"& multi.par$FAST.enrolled=="yes" & multi.par$risk.group=="low"] <- 14
multi.par$par1[multi.par$parameter=="test-freq"& multi.par$FAST.enrolled=="no"] <- NA


out = replicate.epidemic(n.reps = 10,
                         virus.par = virus.par,
                         input.par = multi.par,
                         input.pop= c(7500,7500,2500,2500), #c(20000)
                         n.init.exposed.vector=c(5,5,45,45),#c(100)
                         times=365*2,
                         bay.area.prev = .1/100,
                         bay.area.R = 1.5,
                         burnin = 0,
                         ...
                         serial_sd = 2.3,
                         length_timestep=1,
                         employ.id.vector = c(1, 7501,15001,17501),
                         superspread=TRUE,
                         LOD=(10^1),
                         titer.dat = titer.dat,
                         test_rotation_name = "biweekly-high-two-week-low")

save(out, file = "biweekly-high-two-week-low.Rdata")
