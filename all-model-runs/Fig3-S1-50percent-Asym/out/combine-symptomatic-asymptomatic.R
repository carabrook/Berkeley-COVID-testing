...
setwd("/Users/caraebrook/Documents/R/R_repositories/Berkeley-Reopening/Dec-2020/all-runs/FigS2-50percent-Asym/out/")

load("symptom-iso-1-50asym.Rdata")
dat1 <- out[[1]]
dat1$iso_lag = 1

load("symptom-iso-2-50asym.Rdata")
dat2 <- out[[1]]
dat2$iso_lag = 2

load("symptom-iso-3-50asym.Rdata")
dat3 <- out[[1]]
dat3$iso_lag = 3

load("symptom-iso-4-50asym.Rdata")
dat4 <- out[[1]]
dat4$iso_lag = 4

load("symptom-iso-5-50asym.Rdata")
dat5 <- out[[1]]
dat5$iso_lag = 5
...
#and add in before
setwd("/Users/caraebrook/Documents/R/R_repositories/Berkeley-Reopening/Dec-2020/all-runs/Fig3-Testing-TAT-LOD-Freq/out/")
...
setwd("/Users/caraebrook/Documents/R/R_repositories/Berkeley-Reopening/Dec-2020/all-runs/FigS2-50percent-Asym/out/")
save(dat, file = "dat.all.high.asym.12.29.Rdata")
...
load("/Users/caraebrook/Documents/R/R_repositories/Berkeley-Reopening/Dec-2020/all-runs/Fig2-Group-Limits/out/no-group-lim.Rdata")
dat1 <- out[[4]]
dat.R0.add <- get.rel.R0(dat1)
head(dat.test.R0.hold)
dat.R0.add$iso_lag = Inf
...
dat.test.R0$intervention_class <- "symptom-isolation"
dat.R0.add$intervention_class <- "none"

dat.test.R0 <- rbind(dat.test.R0, dat.R0.add)

dat.test.R0$prop_asym = .51

dat.test.R0.hi = dat.test.R0

#and now, compare the equivalent for the symptom iso with lower prop asym
setwd("/Users/caraebrook/Documents/R/R_repositories/Berkeley-Reopening/Dec-2020/all-runs/Fig3-Testing-TAT-LOD-Freq/out/")
load("symptom-iso-1.Rdata")
dat1 <- out[[4]]
dat1$iso_lag = 1
...
dat.test.R0$LOD[dat.test.R0$LOD==10] <- "1e+01"
dat.test.R0$intervention_class = "symptom-isolation"
dat.test.R0$prop_asym = .32
dat.test.R0.low <- dat.test.R0


dat.test.R0 <- rbind(dat.test.R0.low, dat.test.R0.hi)

#and save and plot.
setwd("/Users/caraebrook/Documents/R/R_repositories/Berkeley-Reopening/Dec-2020/all-runs/FigS2-50percent-Asym/out/")
save(dat.test.R0, file = "R0.all.high.asym.12.29.Rdata")
