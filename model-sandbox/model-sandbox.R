...
#setwd("~/Desktop/berkeley/Boots Lab/Covid testing/Berkeley-COVID-testing/model-sandbox")
...
ITERATION_NUMBER <- 50 #number of realizations of the model, recommend at least 10. 1 run takes ~30 seconds

GROUP_SIZE_LIMIT <- 15
GROUP_SIZE_COMPLIANCE <- 0.9
TURN_AROUND_TIME <- 1 #days ebtween test and result
TRACE_TIME <- 1 #days to complete contact tracing
TRACED_PROPORTION <- 0.9 #fraction of contacts which are traced
SYMPTOMATIC_ISO_TIME <- 1 #days to isolate symptomatic individuals
TEST_ROTATION <- "biweekly" #"biweekly" "weekly" or "two-week" be sure to change DAYS_BW_TESTS as well, automated now
TEST_DAYS_PER_WEEK <- 2
SCHOOL_SIZE <- 650
INIT_INFECTED <- 5 #number of seed cases
TESTING <- TRUE #if false no testing is done
DAYS_BW_TESTS <- 3
if (TEST_ROTATION == "weekly"){ #attempt to make sure TEST_ROTATION and DAYS_BW_TESTS are harmonized
  DAYS_BW_TESTS <- 7
} else if (TEST_ROTATION == "biweekly"){
  DAYS_BW_TESTS <- 3
} else if (TEST_ROTATION == "two-week"){
  DAYS_BW_TESTS <- 14
}
COMMUNITY_PREVALENCE <- .64/100 
COMMUNITY_Reff <- 0.8
NUM_DAYS <- 365 #duration of simulation

OUTPUT_PLOT_NAME <- "Example"
...
make.plots <- function(out, plot_name){
  #daily exposures
  p1 <- ggplot(data=out[[1]]) +
    geom_ribbon(aes(x=day,ymin=lci_exposures, ymax=uci_exposures),  alpha=.3, show.legend = F) + 
    geom_line(aes(x=day, y= mean_exposures), color="black", show.legend = F) + theme_bw() +
    ...
    ylab("daily cases") + xlab("day of epidemic")
  
  print(p1)
  
  #cumulative exposures
  p2 <- ggplot(data=out[[1]]) + 
    geom_line(aes(x=day, y=mean_cumulative), color="black", show.legend = F) +
    geom_ribbon(aes(x=day, ymin=lci_cumulative, ymax=uci_cumulative), alpha=.3, show.legend = F) +
    ylab("cumulative cases") + theme_bw() +  xlab("day of epidemic") +
    theme(panel.grid  = element_blank(), legend.title = element_blank(), legend.position = "bottom", strip.background = element_blank(),
          strip.text = element_blank(), axis.title = element_text(size=14), axis.text = element_text(size=12),  
          plot.margin = unit(c(.5,.5,.5,.5), "lines")) 
  ...
  #daily incidence
  p3 <- ggplot(data=out[[1]]) + 
    geom_line(aes(x=day, y=mean_isolations), color="black", show.legend = F) +
    geom_ribbon(aes(x=day, ymin=lci_isolations, ymax=uci_isolations), alpha=.3, show.legend = F) +
    ylab("daily isolations") + theme_bw() +  xlab("day of epidemic") +
    theme(panel.grid  = element_blank(), legend.title = element_blank(), legend.position = "bottom", strip.background = element_blank(),
          strip.text = element_blank(), axis.title = element_text(size=14), axis.text = element_text(size=12),  
          plot.margin = unit(c(.5,.5,.5,.5), "lines")) 
  ...
  p4 <- ggplot(data=out[[1]]) + 
    geom_line(aes(x=day, y=mean_cumulative_isolations), color="black", show.legend = F) +
    geom_ribbon(aes(x=day, ymin=lci_cumulative_isolations, ymax=uci_cumulative_isolations), alpha=.3, show.legend = F) +
    ylab("cumulative isolations") + theme_bw() +  xlab("day of epidemic") +
    theme(panel.grid  = element_blank(), legend.title = element_blank(), legend.position = "bottom", strip.background = element_blank(),
          strip.text = element_blank(), axis.title = element_text(size=14), axis.text = element_text(size=12),  
          plot.margin = unit(c(.5,.5,.5,.5), "lines")) 
  ...
  ggsave(file = paste(OUTPUT_PLOT_NAME,".png"),
         units="mm",  
         width=100, 
         height=100, 
         scale=3, 
...
#Update parameter dataframe with new given values
pop.par.base$par1[pop.par.base$parameter=="test-on"] <- TESTING
pop.par.base$par1[pop.par.base$parameter=="test-freq"] <- DAYS_BW_TESTS
pop.par.base$par1[pop.par.base$parameter=="group-size-limit"] <- GROUP_SIZE_LIMIT
pop.par.base$par1[pop.par.base$parameter=="percent-obs-dist-lim"] <- GROUP_SIZE_COMPLIANCE
pop.par.base$par1[pop.par.base$parameter=="trace-lag"] <- TRACE_TIME
pop.par.base$par1[pop.par.base$parameter=="prop.trace"] <- TRACED_PROPORTION
pop.par.base$par1[pop.par.base$parameter=="iso-lag"] <- SYMPTOMATIC_ISO_TIME
pop.par.base$par1[pop.par.base$parameter=="test-rotation"] <- TEST_ROTATION
pop.par.base$par1[pop.par.base$parameter=="n-test-days-per-week"] <- TEST_DAYS_PER_WEEK
pop.par.base$par1[pop.par.base$parameter=="TAT-lag"] <- TURN_AROUND_TIME

#subset titer data frame to contain correct number of individuals
titer.dat <- subset(titer.dat, employ_ids <= SCHOOL_SIZE)

#run the model
out = replicate.epidemic(n.reps = ITERATION_NUMBER,
                         virus.par = virus.par,
                         input.par = pop.par.base,
                         input.pop=c(SCHOOL_SIZE),#2000
                         n.init.exposed.vector=c(INIT_INFECTED),#10
                         times=NUM_DAYS,
                         bay.area.prev = COMMUNITY_PREVALENCE,
                         bay.area.R = COMMUNITY_Reff,
                         burnin = 0,
                         serial_mean = 4.8,
                         serial_sd = 2.3,
                         length_timestep=1,
...
#save output
save(out, file = paste(OUTPUT_PLOT_NAME,".Rdata"))
#make and save the basic plot
make.plots(out,OUTPUT_PLOT_NAME)
