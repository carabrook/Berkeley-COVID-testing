...
#setwd("~/Desktop/berkeley/Boots Lab/Covid testing/Berkeley-COVID-testing/model-sandbox/multi-population")
...
POP1_SIZE <- 5000
POP1_INIT_INFECTED <- 10
POP2_SIZE <- 5000
POP2_INIT_INFECTED <- 10
...
multi.par$par1[multi.par$parameter=="group-size-limit"] <- GROUP_SIZE_LIMIT
multi.par$par1[multi.par$parameter=="percent-obs-dist-lim"] <- GROUP_SIZE_COMPLIANCE
...
out = replicate.epidemic(n.reps = 100,
                         virus.par = virus.par,
                         input.par = multi.par,
                         input.pop= c(POP1_SIZE,POP2_SIZE),#2000
                         n.init.exposed.vector=c(POP1_INIT_INFECTED,POP2_INIT_INFECTED),#10
                         times=NUM_DAYS,
                         ...
                         burnin = 0,
                         length_timestep=1,
                         employ.id.vector = c(1, POP1_SIZE+1),
                         LOD=(10^1),
                         titer.dat = titer.dat,
                         test_rotation_name = "weekly-all")

save(out, file = paste(OUTPUT_NAME,".Rdata"))
